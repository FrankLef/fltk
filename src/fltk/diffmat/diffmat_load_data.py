...
def load_data(inst:DiffMat, data:pd.DataFrame, idx_var:str, value_var:str, group_vars: Iterable[str])->None:
...
def validate_data_keys(inst:DiffMat,data:pd.DataFrame,idx_var:str, group_vars:Iterable[str])->None:
    keys = [idx_var]
