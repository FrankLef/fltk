...
def main(bag_nm: str, data: pl.DataFrame) -> DiczBag:
    dicts: GroupDict = get_dicts.main(data)
    bag: DiczBag = build_bag(bag_nm=bag_nm, dicts=dicts)
    nlines = sum([len(x) for x in dicts.values()])
    if bag.nlines != nlines:
        msg: str = (
            f"Bag '{bag_nm}' has {bag.nlines} lines. It must have {nlines} lines."
        )
        raise AssertionError(msg)
    return bag


def build_bag(bag_nm: str, dicts: GroupDict) -> DiczBag:
    dicz_bag = DiczBag(name=bag_nm)
    for group_nm, group_val in dicts.items():
        dicz_group = DiczGroup(name=group_nm)
        for line_nm, line_val in group_val.items():
            dicz_line = DiczLine(name=line_nm)
            for item_nm, item_val in line_val.items():
                dicz_item = DiczItem(name=item_nm, value=item_val)
